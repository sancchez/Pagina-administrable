...
export default function AdminEditor() {
  ...
  const fetchPage = async (pageId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPage(data);
        setTitle(data.title);
        // Extraer contenido del draft_json
        const draftContent = data.draft_json?.blocks?.map((block: any) => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // Convertir HTML a estructura de bloques simple
      const blocks = content.split('\n').filter(line => line.trim()).map(line => {
        if (line.includes('<h1>')) {
          return {
            type: 'header',
            data: { level: 1, text: line.replace(/<\/?h1>/g, '') }
          };
        }
        if (line.includes('<h2>')) {
          return {
            type: 'header',
            data: { level: 2, text: line.replace(/<\/?h2>/g, '') }
          };
        }
        return {
          type: 'paragraph',
          data: { text: line.replace(/<\/?p>/g, '') }
        };
      });
      ...
      if (response.ok) {
        ...
        alert(`Error al guardar la página: ${errorData.message || 'Error desconocido'}`);
      }
    } catch (error) {
    ...
    <div className="h-[calc(100vh-97px)] flex flex-col">
            ...
            <div className="flex items-center space-x-4">
              <div>
                ...
                <div className="text-sm text-gray-500 mt-1">
                  {isEditing ? `Editando: ${page?.id}` : 'Nueva página'}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-gray-50 px-6 py-3 border-b border-gray-200">
        <div className="text-sm text-gray-600">
          {page ? `Estado: ${page.status} | Versión: ${page.version}` : 'Nueva página en borrador'}
        </div>
      </div>
