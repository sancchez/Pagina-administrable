...
export default function PageVersionManager({ pageId, onVersionRestore }: PageVersionManagerProps) {
  ...
  const loadVersions = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/page-versions/page/${pageId}`);
      if (response.ok) {
  ...
  const createOriginalVersion = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/page-versions/page/${pageId}/original`, {
        method: 'POST',
        headers: {
  ...
  const restoreVersion = async (versionId: number) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/page-versions/${versionId}/restore`, {
        method: 'PUT'
      });
  ...
  const setAsOriginal = async (versionId: number) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/page-versions/${versionId}/set-original`, {
        method: 'PUT'
      });
