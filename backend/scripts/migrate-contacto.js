...
async function migrateContacto() {
  try {
    console.log('Iniciando migración de la página Contacto...');
    ...
    const result = await prisma.page.upsert({
      ...
      update: {
        title: 'Contacto',
        content: htmlContent,
        metaDescription: 'Contáctanos para cualquier consulta, reclamo o emergencia. Múltiples canales de atención disponibles 24/7.',
        published: true
      },
      create: {
        slug: 'contacto',
    ...
    console.log('✅ Página Contacto migrada exitosamente:', result.title);
  } catch (error) {
    console.error('❌ Error al migrar la página Contacto:', error);
  } finally {
